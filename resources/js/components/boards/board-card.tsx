...
export const BoardCard = forwardRef<HTMLDivElement, BoardCardProps>(
    function BoardCard(
        { card, onOpen, className, onClick, ...props },
        ref,
    ) {
        return (
            <div
                    ...
                    {card.github_link && (
                        <span className="flex items-center gap-1.5">
                            ...
                            >
                                ...
                                <span
                                    className={`rounded-full px-1.5 py-px text-[10px] font-medium capitalize ${
                                        card.github_link.state === 'open'
                                            ? 'bg-green-100 text-green-700'
                                            : 'bg-purple-100 text-purple-700'
                                    }`}
                                >
                                    {card.github_link.state}
                                </span>
                            </a>
                            {card.github_link.pull_request_number && (
                                <a
                                    href={
                                        card.github_link.pull_request_url ??
                                        '#'
                                    }
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-0.5 hover:text-foreground"
                                    title={`Pull request #${card.github_link.pull_request_number} (${card.github_link.pull_request_state ?? 'open'})`}
                                    onClick={(e) => e.stopPropagation()}
                                >
